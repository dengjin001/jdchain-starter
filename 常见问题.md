...
# 22.JD-chain支持最多一个节点能够提供多少合约的代理服务？有限制吗？
在调用合约的时候，首先需要通过合约地址获得合约代理对象。那么如果链上发布了多个合约，这个代理能力就会变多。那么server端的连接是否会随着合约数量增多而变多呢？
## 热心反馈者
文彬 时间：2020.2.28   
## 解决
server端是一个可复用的连接池，顺序调用。网络连接数跟合约数量无关。在设计上没有限制一个节点只能有多少合约，只受到系统资源的限制。目前我们没有做这方面的压力测试。
# 21.如何获取一个合约的具体操作历史？
我们想获取执行合约这种类型的交易列表，想看看具体的一个合约的操作历史，jdchain 有接口能直接获取吗？从区块遍历的方式有点麻烦。
## 热心反馈者
姜文超 时间：2020.2.24   
## 解决
区块链暂时无法提供丰富的查询方式，我们有一个实验性的产品模块“穿透式检索系统”可以提供这种能力，但是目前还是实验阶段，还没正式发布。  
可行的解决方案是，我们建议应用层自己做一些关键索引。毕竟交易都是从应用端发起的，应用端在记录交易相关的字段时候加上一些额外的属性。  
# 20.两台16核64G的服务器，如何部署JD Chain？
现在有两台16核64G的服务器，您觉得需要考虑区块链会宕掉而把peer部署成分布式，分布到两台机器上吗？如果要是部署的话，您觉得每个服务器部署几个peer会比较好呢？
## 热心反馈者
付一健 时间：2020.2.20   
## 解决
2台服务器和1台服务没有差别，宕机一半都无法工作。如果有可能的话，最好4个节点的服务器都是独立的，这样才能容错。 BFT 共识的容错率计算公式是 总节点数 n = 3 * f + 1，f 为容错节点数。4 个节点的情况下， 能够承受 1 个节点宕机后整个网络仍然能够达成共识。 实际需要根据项目实际情况和风险要求，恰当选择服务器的数量和共识节点进程的分布。按理来说，应该是每个参与方都有自己的服务器来部署自己的共识节点，服务器是由整个网络的参与方来共同分摊成本的。
# 19.JD-chain合约详情里面的rootHash是作为合约源码一致性校验使用的么？
## 热心反馈者
文彬 时间：2020.2.19  
## 解决
合约源码一致性校验，你这个理解对。是对chaincode等几个参数组合生成的hash。如果源码不一致，chaincode也就变了，rootHash也会变化。
# 18.版本1.1.2版本下的schema.retrieval.url该如何配置
我是青岛的java开发Yule.最近在看jdchain,能请教您一下，1.1.2版本下的schema.retrieval.url该如何配置吗？谢谢
## 热心反馈者
蒲聿（puyu），时间：2020.1.14
## 解决
````
需要关注两个文件：
1）gateway.conf
...
2）manager-booter-1.1.2.RELEASE.jar -》application.properties
//由于管理工具中也用到了穿透式检索，需要进行对应的设置，目前设置为：localhost，根据需要修改对应的IP值：
schema.retrieval.url=http://127.0.0.1:8082
